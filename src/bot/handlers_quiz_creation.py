...
async def _ask_mode(message: Message, state: FSMContext):
    ...
    await message.answer("Send questions one-by-one or paste all at once. Reply `single` or `bulk`.")
...
async def mode_selected(message: Message, state: FSMContext):
    ...
    if mode == "single":
        ...
        await message.answer("Send question text (see formats). Send /done when finished adding questions.")
    else:
        await state.set_state(NewQuizStates.waiting_bulk_content)
        await message.answer("Paste all questions in the supported bulk format. Then send /parse.")
...
async def single_done(message: Message, state: FSMContext):
    ...
    if not data.get("questions"):
        await message.answer("No questions added yet. Please add at least one or cancel.")
        return
...
async def single_add_question(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Added Q{idx+1}: {pq.text}\nOptions: " + ", ".join([f"{i+1}) {o}" for i, o in enumerate(pq.options)]) + f"\nAnswer: {pq.options[pq.correct_index]}"
    )
...
async def bulk_parse_command(message: Message, state: FSMContext):
    ...
    for i, pq in enumerate(preview, 1):
        preview_lines.append(f"{i}. {pq.text} \n - Ans: {pq.options[pq.correct_index]}")
    await state.update_data(parsed_bulk=[{
        "text": p.text,
        "options": p.options,
    ...
    await message.answer(
        "\n".join(["Preview:"] + preview_lines + [f"Total parsed: {len(parsed)} {err_line}", "Proceed? (yes/no)"])
    )
...
async def _ask_duration(message: Message, state: FSMContext):
    ...
    await message.answer("Set duration (in minutes). Send 0 or unlimited for no time limit.")
...
async def duration_received(message: Message, state: FSMContext):
    ...
    if t in {"0", "unlimited", "none"}:
        ...
        try:
            ...
            await message.answer("Please send a number of minutes, or 0/unlimited.")
            return
    await state.update_data(duration_minutes=minutes)
    await state.set_state(NewQuizStates.waiting_visibility)
    await message.answer("Shall this quiz be public or private? Send `public` or `private`.")


@router.message(NewQuizStates.waiting_visibility, F.text.lower().in_({"public", "private"}))
...
async def confirm_create(message: Message, state: FSMContext):
    ...
    async with session_factory() as session:
        ...
        await message.answer(
            f"Quiz created!\nTitle: {quiz.title}\nQuestions: {len(data.get('questions', []))}\nDuration: {data.get('duration_minutes')} min\nLink: {link}",
